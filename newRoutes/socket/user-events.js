...
//	{sendInProgressGameUpdate} = require('./util.js');
    ...
    getPrivatePlayerInGameByUserName = (game, userName) => game.private.seatedPlayers.find(player => player.userName === userName);
...
sendInProgressGameUpdate = (game, socket) => { // todo-release make this accept a socket argument and emit only to it if it exists
...
console.log(playerSockets);
	if (playerSockets) {
		playerSockets.forEach(sock => {
			...
			console.log("passport", sock.handshake.session.passport)
			console.log("user", sock.handshake.session.passport)
